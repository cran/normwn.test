...
n<- as.double(nrow(x))
 ...
 for (j in 1:nvars)
 ac[,j]<- acf(trprime[,j],lag.max=n-1,type="covariance")$acf
 F3<- c(numeric(nvars))
 F4<- c(numeric(nvars))
 for (j in 1:nvars)
 F3[j]<- ac[1,j]^3 + 2*sum(ac[2:n,j]^3)
 for (j in 1:nvars)
 F4[j]<- ac[1,j]^4 + 2*sum(ac[2:n,j]^4)
 rtb1<- c(numeric(nvars))
 b2<- c(numeric(nvars))
 for (j in 1:nvars)
 rtb1[j]<- v3[j]/sqrt(F3[j])
 for (j in 1:nvars)
 b2[j]<- v4[j]/sqrt(F4[j])
 print("rtb1")
 print(rtb1)
 ...
 aye<- ((n-2)*(n+5)*(n+7)*(n^2+27*n-70))/(6*del)
 cee<- ((n-7)*(n+5)*(n+7)*(n^2+2*n-5))/(6*del)
 alp<- aye+((rtb1^2)*cee)
 kap<- ((n+5)*(n+7)*(n^3+37*n^2+11*n-313))/(12*del)
 ...
 for (p in 1:nvars)
 pvalsk[p]<- pnorm(z1[p],lower.tail=FALSE)
 for (p in 1:nvars)
 pvalsk[p]<- 2*pvalsk[p]
 for (p in 1:nvars)
 if (pvalsk[p]>1) pvalsk[p]<- 2-pvalsk[p]
 ...
 for (p in 1:nvars)
 pvalk[p]<- pnorm(z2[p],lower.tail=FALSE)
 for (p in 1:nvars)
 pvalk[p]<- 2*pvalk[p]
 for (p in 1:nvars)
 if (pvalk[p]>1) pvalk[p]<- 2-pvalk[p]
